	...
	Templates:new Class({
		...
		createTemplate:function(name,container,fullHeight){
			...
			parent.addClass('ajaxContainer').addEvents({
				...
				*/
				...
				onAfterFullHeight:function(){
					parent.setStyle('max-height',parent.getStyle('height'));
				...
				if (template.dynamic) {
					...
					this.updateTemplate(name).renderTemplate(name);
				...
				if ($defined(template.dynamic)) {
					if (template.dynamic) {
						if ($defined(template.itemHeight)) {
							template.containment.setStyles({
								height:template.items.length*template.itemHeight,
		...
		renderTemplate:function(name,onRender){
			...
			if (template.dynamic) {
				...
				template.container.empty().setStyle('margin-top',0);
				var ratio = scroll.y?scroll.y/(count*itemHeight):0;
				var start = (count*ratio).toInt();
				template.$offset = start;
				for (var i=start;i<count;i++) {
					var position = itemHeight*i;
					if ((position+itemHeight>=scroll.y) && (position<=scroll.y+parentHeight+itemHeight)) {
						if ($empty && scroll.y) {
							template.container.setStyle('margin-top',position-scroll.y);
							$empty = false;
						}
						var item = template.items[i];
						var el = this.createTemplateElement(template,item.data).inject(template.container);
						
						var iHeight = el.getCoordinates().height;
						if (iHeight>itemHeight) {
							itemHeight = iHeight;
						}
						
						if ($type(item.onCreateElement)=='function') {
							item.onCreateElement(el,template,item.data);
						} 
					} 
					if (position>scroll.y+parentHeight+itemHeight) {
						break;
					}
				}
				if (itemHeight!=template.itemHeight) {
		...
		createTemplateElement:function(template,data){
			var el = new Element(template.tag)
							...
							.set('html',template.template.substitute($pick(data,{})))
							;
			...
			el.getElements('.styleContent').each(function(el){
				el.setStyle(el.get('data-style'),el.get('data-content'))
					.removeProperties('data-style','data-content').removeClass('styleContent')
					;
			});
		...
		applyTemplate:function(name,data,onCreateElement){
			var template = this.$templates[name];
			if (!$defined(template.itemHeight) && template.dynamic) {
				var el = this.createTemplateElement(template,data).inject(template.container);
				template.itemHeight = el.getCoordinates().height;
				el.remove();
			}
			template.items.push({
				data:data,
...
TPH.validateForm = function(form){
	...
	for(var i=0;i<count;i++) {
		...
		if ((!field.validity.valid || (field.get('type')=='hidden' && field.hasClass('required') && field.get('value').trim()=='')) && !field.disabled) {
...
TPH.Dropdown = new Class({
	...
	position:function(dd){
		...
		if (coords.bottom>winSize.y) {
			dd.setStyle('height',coords.height-(coords.bottom-winSize.y)-26);
		} else {
	...
	read:function(files){
		if (files.length) {
			...
			reader.onloadend = function(e){
				...
				this.fireEvent('onReadFile',[fileData,this,e]);
...
TPH.uploadButton = new Class({
	...
	initialize:function(el,options){
		...
		this.setOptions(options);
		this.$index = 0;
		this.el.addEvent('input',function(e){
			e.stop();
			this.content.empty();
			var files=[],
				count = this.el.files.length;
...
TPH.TreeList = new Class({
	...
	buildTree:function(){
		...
		for(primaryField in index){
			...
			if (item[this.options.parentField].toInt()) {
				index[item[this.options.parentField]].children.push(item);
			}
		}
		this.buildList();
	},
	...
	applyItem:function(item,level){
		item.level = this.options.separator.repeat(level);
		item.expandMarker = item.children.length?(item.$expanded?this.options.expanded:this.options.collapsed):this.options.separator;
...
TPH.QRScanner = new Class({
	...
	capture:function(){
		...
		try {
			...
			if (err == -2) {
			    this.$captureTimer = this.capture.delay(30,this);
			} else {
...
function $fullHeight(container){	
		...
		if (!['absolute','fixed'].contains(el.getStyle('position'))) {
			if (!['svg'].contains(el.get('tag'))) {
				...
				if (el.hasClass('fullHeight') && !isAutoHeight && display!='none'){ //el.getStyle('height')!='auto') {
					...
					offset+=el.getStyle('margin-top').toInt()+el.getStyle('margin-bottom').toInt()+el.getCoordinates().height;
				}	
			} else {
				el.setStyle('display','');
			}	
	...
	els.each(function(el){		
		...
		if (targetHeight>(container.clientHeight-paddings-borders-el.getStyle('margin-top').toInt()-el.getStyle('margin-bottom').toInt())) {
			targetHeight = container.clientHeight-paddings-borders-el.getStyle('margin-top').toInt()-el.getStyle('margin-bottom').toInt();
